...
def ta_signals(spread, idx, extra={}):
    # All signals numeric, normalized
    sma = SMA(spread, 10)
    ema = EMA(spread, 5)
    rsi = RSI(spread, 7)
    ...
    # Voting: +1 if signal >0, -1 if signal<0, 0 if neutral
    votes = {k: (1 if v>0.08 else -1 if v<-0.08 else 0) for k,v in signals.items()}
    # For narrative, build a summary of votes & strengths
...
with col2:
    # Output windows for recs, TA, best bets, etc.
    if clear:
    ...
    if analyze and blocks:
        ...
        ta_table = pd.DataFrame({
            "Signal": signals,
            "Weight": [WEIGHTS.get(k,0) for k in signals],
            "Vote": votes
        }).T
        st.dataframe(ta_table)
        # Projections
        ...
        # Simplified backtest: for demo, bet 5% bankroll on top play each tick.
        df = pd.DataFrame(blocks)
        df = df.sort_values("time").reset_index(drop=True)
        br = br_val
        br_hist = [br]
        ...
        for i in range(2, len(df)-1):
            ...
            stake = br * kf
            # Simulate: win if direction matches spread change sign at next tick
        ...
        st.dataframe(pd.DataFrame(bet_hist))
