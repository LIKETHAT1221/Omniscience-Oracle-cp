...
# ----------- DATETIME FIX -------------
def ensure_datetime_format(df):
    if 'time' in df.columns:
        print(f"Original time column type: {df['time'].dtype}")
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    print(f"All datetime conversion attempts failed: {e}")
                    raise
        print(f"Converted time column type: {df['time'].dtype}")
        print(f"Sample converted values: {df['time'].head(3).tolist()}")
    return df
...
# ----------- RECENCY WEIGHTS (ROBUST) -------------
def recency_weights(times, recency_half):
    try:
        print(f"First 3 times: {times[:3]}")
        print(f"Types of first 3 times: {[type(t) for t in times[:3]]}")
        times_converted = []
        for t in times:
            try:
                if isinstance(t, str):
        ...
        print(f"First 3 converted times: {times[:3]}")
        print(f"Types after conversion: {[type(t) for t in times[:3]]}")
        latest_time = max(times)
        print(f"Latest time: {latest_time} (type: {type(latest_time)})")
        deltas = []
        for t in times:
            try:
                ...
                raise
        weights = [2 ** (-delta / recency_half) for delta in deltas]
        norm_weights = np.array(weights) / (np.sum(weights) + 1e-12)
        return norm_weights.tolist()
    except Exception as e:
        print(f"Critical error in recency_weights: {e}")
